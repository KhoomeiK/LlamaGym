...
class Agent(ABC):
    ...
    def llm(self, messages) -> str:
        ...
        generate_ids = self.model.generate(
            ...
            **{key.split('/')[-1]: value for key, value in self.generate_config_dict.items()}
        )
    ...
    def terminate_episode(self):
        queries, responses, rewards = self.format_episode_for_ppo(self.current_episode_messages, self.current_episode_rewards)
        ...
        if len(self.current_batch["queries"]) >= self.ppo_config.batch_size:
            train_stats = self.train_batch(self.current_batch["queries"], self.current_batch["responses"], self.current_batch["rewards"])
            return train_stats
